...
void precheck(FILE *file_descp, char *buff, long long int x, long long int size,
...
void loopcheck(FILE *file_descp, char *buff, long long int temp_x,
               long long int temp_size, long long int temp_y,
               long long int temp_z, long long int x, long long int size,
               long long int y, long long int z) {
  int f = loopflag;
  ...
  if (f == 0 && loopflag == 1) {
    fprintf(file_descp,
            ...
            temp_x, "size", temp_size, "y", temp_y, "z", temp_z);
    fprintf(file_descp,
...
int main() {
  ...
  for (;;) {
    ...
    snprintf(vars, 128, "%s : %lld, %s : %lld, %s : %lld, %s : %lld", "x", x,
             "size", size, "y", y, "z", z);
    ...
    if (choices > 10000) {
      ...
      if ((x < size)) {
        ...
        while ((x < size) && k--) {
          ...
          loopcount++;
          loopcheck(fptr, vars, temp_x, temp_size, temp_y, temp_z, x, size, y,
                    z);
        }
      } else {
        // post-check program
