...
/**
 * Rewriter Class- A class to rewrite the code to better suit the requirements
 * for the SSA
 */
...
private:
  ...
  /**
   * simplifyNot- A function to simplify conditions surrounded by not, like !(c1
   * && c2) which interferes with the SSAWriter's functioning
   *
   * @param expr
   * @param notLevel
...
public:
  ...
  RewriterClass(clang::ASTContext *astContext, clang::Rewriter &rewriter)
      : context(astContext), rewriter(rewriter) {}
};

class RewriterConsumer : public clang::ASTConsumer {
public:
  explicit RewriterConsumer(clang::ASTContext *Context,
                            clang::Rewriter &rewriter)
