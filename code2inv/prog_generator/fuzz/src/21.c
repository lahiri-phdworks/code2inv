...
void precheck(FILE *file_descp, char *buff, long long int x, long long int m,
              long long int n, long long int z1, long long int z2,
              long long int z3) {
  int f = preflag;
...
void loopcheck(FILE *file_descp, char *buff, long long int temp_x,
               long long int temp_m, long long int temp_n, long long int x,
               long long int m, long long int n, long long int z1,
               long long int z2, long long int z3) {
  int f = loopflag;
  aflcrash(INV(x, m, n, z1, z2, z3), loopflag);
  if (f == 0 && loopflag == 1) {
    fprintf(file_descp, "LoopStart : %s : %lld, %s : %lld, %s : %lld\n", "x",
            temp_x, "m", temp_m, "n", temp_n);
...
{                                                                           \
   ...
   aflcrash(cond, postflag);                                                   \
    ...
    if (f == 0 && postflag == 1) {                                             \
      \ 
        fprintf(                                                               \
          file_descp, "Post : %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld\n", \ 
        "x",                                                                   \
