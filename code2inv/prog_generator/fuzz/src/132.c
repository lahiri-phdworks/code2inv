...
void loopcheck(FILE *file_descp, char *buff, long long int temp_i,
               long long int temp_j, long long int temp_c, long long int temp_t,
               long long int i, long long int j, long long int c,
               long long int t) {
  int f = loopflag;
  ...
  if (f == 0 && loopflag == 1) {
    fprintf(file_descp,
            ...
            temp_i, "j", temp_j, "c", temp_c, "t", temp_t);
    fprintf(file_descp,
...
int main() {
  ...
  for (;;) {
    ...
    snprintf(vars, 128, "%s : %lld, %s : %lld, %s : %lld, %s : %lld", "i", i,
             "j", j, "c", c, "t", t);
