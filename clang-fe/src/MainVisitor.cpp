...
bool MainFunctionVisitor::VisitFunctionDecl(clang::FunctionDecl *Declaration) {
  if (Declaration->isMain()) {
    ...
    cfg = CFG::buildCFG(Declaration, body, Context, CFG::BuildOptions())
              .release();
    ...
    // IMPORTANT
    // The buildDominatorTree method gives a SegFault for two of the 19 programs
    // under the tests directory for due to mysterious reasons Needs to be fixed
    ...
    dominanceFrontier.calculateFrontiers(dominatorTree.getRootNode(),
                                         dominatorTree.DT);
    ...
    // to fix this the rewriter edits the code such that there is only one decl
    // per declstmt
    cm = analysisDeclContext.getCFGStmtMap();

    // visitDeclStmt is used for getting the list of variables, their
    // assignments and mapping the CFGBlocks to the variable occurrences
    ...
    phiPlacements = placePhi(assignments, dominanceFrontier.getFrontier(),
                             *dominatorTree.DT);

    /*
    for(const auto &pair : phiPlacements) {
        llvm::errs() << "Phi functions in block " << pair.first->getBlockID() <<
    " for: "; for(const auto &varNumArgPair : pair.second) { llvm::errs() <<
    "variable " << varNumArgPair.first << " with " <<
    varNumArgPair.second.size() -1 << " args ";
        }
        llvm::errs() << "\n";
    }
    ...
    for(const auto &pair : blockVariableMap) {
        llvm::errs() << "Variables, statements and locs for block " <<
    pair.first->getBlockID() << "\n"; for(const auto &varStmtCollectn :
    pair.second) { llvm::errs() << varStmtCollectn.var << "\n";
            varStmtCollectn.stmt->dump();
            llvm::errs() <<
    varStmtCollectn.begin.printToString(Context->getSourceManager()) << " "
                         <<
    ...
    */
    ...
    // mapping the block ids to the phi functions instead of the CFGBlocks
    // themselves this is because if we map CFGBlock* to the functions, when the
    // pass is complete, the CFGBlock* addresses are no longer valid
    for (const auto &pair : phiPlacements) {
...
MainFunctionVisitor::varStmtStruct::varStmtStruct(
    std::string varName, clang::Stmt *varStmt, clang::SourceLocation varBegin,
    clang::SourceLocation varEnd, bool isLvalue, clang::ASTContext *ctx) {
...
/**
 * We define a less than condition for the varStmtStruct so that when traversing
 * a set of these structs, the variables are considered from the top of the code
 * to the bottom, but in each line they are viewed from left to right.
 * @param x
 * @return a bool
 */
...
bool MainFunctionVisitor::varStmtStruct::operator==(
    const MainFunctionVisitor::varStmtStruct &x) const {
  return var == x.var && stmt == x.stmt && begin == x.begin && end == x.end &&
         islvalue == x.islvalue;
}

/**
 * printDeclStmt- Is not really supposed to print anything, but to get the list
 * of variables into the variables vector, information about said variable in
 * blockVariableMap and the blocks where the variable gets assigned a value in
 * the assignments map.
 * @param stmt
 * @param cfg
 */
void MainFunctionVisitor::visitDeclStmt(clang::Stmt *stmt) {
  if (std::string(stmt->getStmtClassName()) == "DeclStmt") {
    ...
    for (auto iter = declSt->decl_begin(); iter != declSt->decl_end(); iter++) {
      ...
      // causes segfault when more than one vardecl in one stmt since for some
      // reason cm->getBlock(stmt) gives 0x0
      blockVariableMap[cm->getBlock(stmt)].insert(
          varStmtStruct(varName, stmt, varDecl->getLocation(),
                        varDecl->getEndLoc(), true, Context));
    ...
    if (std::string(ref->getDecl()->getDeclKindName()) == "Var") {
      ...
      // this now checks if the variable given by declrefexpr is an LValue or an
      // RValue this is done by checking the variable's parents if it is an
      // assignment operator (= sign), then var is an LValue else RValue
      ...
      for (auto nodes : parents) {
        ...
        if (auto binst = llvm::dyn_cast<BinaryOperator>(st)) {
          if ((binst->getLHS() == ref)) {
            ...
            blockVariableMap[cm->getBlock(stmt)].insert(varStmtStruct(
                ref->getDecl()->getName().str(), binSt, ref->getBeginLoc(),
                ref->getEndLoc(), true, Context));
            break;
          }
        }
      }
      ...
      if (!isLValue) {
        blockVariableMap[cm->getBlock(stmt)].insert(varStmtStruct(
            ref->getDecl()->getName().str(), stmt, ref->getBeginLoc(),
            ref->getEndLoc(), false, Context));
      }
    }
  }
...
int MainFunctionVisitor::whichPred(clang::CFGBlock *X, clang::CFGBlock *Y) {
  int i = 0;
  // we iterate reversed because in Cytron et al. the CFGBlocks are numbered in
  // an ascending order while going from entry to exit block, whereas the
  // Clang's CFGBlocks are numbered in the descending order, resulting in a
  // different value for whichPred
...
void MainFunctionVisitor::search(clang::DomTreeNode *X,
                                 std::map<std::string, int> &C,
                                 std::map<std::string, std::stack<int>> &S) {
  CFGBlock *cfgX = X->getBlock();

  // first check for phi function
  // Cytron et al. considers phi functions to be at the beginning of the
  // concerned block.
  for (auto &pair : phiPlacements[cfgX]) {
    // pair.first is the original variable for which the phi function is
    // considered pair.second is the vector consisting of the LHS of the phi
    // function and the list of arguments If the phi function is so: x_3 =
    // phi(x_2, x_1, x_4), pair.second is a vector of elements x_3, x_2, x_1,
    // x_4
  ...
  for (const auto &varInfo : blockVariableMap[cfgX]) {
    // In Cytron et al. for each statement first the RHS variables are renamed
    // and then the LHS variables get renamed since our < condition for
    // varStmtStruct dictates that in each statement, we traverse right to left,
    // simply iterating over the set lets us replace the variables as needed in
    // the same order as Cytron et al. does
  ...
  for (auto iterY = cfgX->succ_rbegin(); iterY != cfgX->succ_rend(); iterY++) {
    ...
    // Here we again traverse in the reverse order for the same reason we
    // traversed in reverse for whichPred

    auto Y = *iterY;
    // llvm::errs() << "Y is " << Y->getBlockID() << " and X is " <<
    // cfgX->getBlockID() << "\n";
    int j = whichPred(cfgX, Y);
    if (j != -1) {
      for (auto &phiFunc : phiPlacements[Y]) {
        // llvm::errs() << "Before getting top... is stack for " <<
        // phiFunc.first << " empty? " << S[phiFunc.first].empty() << "\n";
        // llvm::errs() << j << " is the position of the argument...\n";
        int i;
        if (S[phiFunc.first].empty()) {
          // IMPORTANT- verify the accuracy of this method
          // Sometimes the algorithm considers a block which has a phi function
          // for a variable it has not encountered an assignment of yet. In such
          // cases, the stack of said variable is empty and so S.top() is
          // undefined, sometimes leading to segmentation faults. A possible fix
          // is to consider this to be the 0th instance of the variable since it
          // has not been declared/assigned yet
          i = 0;
        } else {
          i = S[phiFunc.first].top();
        }
        // llvm::errs() << i << "\n";
        if (std::find(phiFunc.second.begin(), phiFunc.second.end(),
                      phiFunc.first + "_" + std::to_string(i)) ==
            phiFunc.second.end()) {

          phiFunc.second[j + 1] = phiFunc.first + "_" + std::to_string(i);
          variables[phiFunc.first].insert(phiFunc.first + "_" +
                                          std::to_string(i));
          // phiFunc.second.push_back(phiFunc.first + "_" + std::to_string(i));
        }
        /*
        llvm::errs() << "Added variable " << phiFunc.first + "_" +
        std::to_string(i)
                     << " to the phi function to " << phiFunc.second[0] << " in
        block "
...
void MainFunctionVisitor::renameVars(
