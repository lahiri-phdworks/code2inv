...
void precheck(FILE *file_descp, char *buff, long long int lock,
...
void loopcheck(FILE *file_descp, char *buff, long long temp_lock,
               long long temp_x, long long temp_y, long long int lock,
               long long int v1, long long int v2, long long int v3,
               long long int x, long long int y) {
  int f = loopflag;
  ...
  if (f == 0 && loopflag == 1) {
    fprintf(file_descp,
            ...
            "%lld, %s : %lld\n",
            "lock", temp_lock, "v1", v1, "v2", v2, "v3", v3, "x", temp_x, "y",
            temp_y);
    fprintf(file_descp,
    ...
    if (choices > 10000) {
      ...
      if ((x != y)) {
        ...
        while ((x != y) && k--) {
          ...
          loopcount++;
          loopcheck(fptr, vars, temp_lock, temp_x, temp_y, lock, v1, v2, v3, x,
                    y);
        }
      } else {
        // post-check program
