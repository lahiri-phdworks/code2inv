...
class ExprNode(object):
    ...
    def __str__(self):
        ...
        if self.name == '&&' or self.name == '||':
            ...
            if len(child_list):
                st = child_list[0].__str__() + self.name + \
                    child_list[1].__str__()
            else:
                st = self.name
        return st
    ...
    def __eq__(self, othernode):
        if othernode is None:
            return False
        res = (self.name == othernode.name) and (self.pg_node ==
                                                 othernode.pg_node) and (self.children == othernode.children)
        return res
...
class ProgramGraph(object):
    def __init__(self, graph_json):
        ...
        if cmd_args.encoder_model != "Param":
            ...
            for edges in graph_json['control-flow']:
                ...
                self.add_double_dir_edge(
                    x, y, CONTROL_EDGE_TYPE, CONTROL_EDGE_TYPE + 1)
            ...
            if pre_if is not None:
                ...
                self.if_before_assert = self.build_assert(
                    val['rval'], self.core_vars)
            val = graph_json['nodes'][assert_node]
            self.assert_statement = self.build_assert(
                val['rval'], self.core_vars)
    ...
    def add_ssa_variables(self, d):
        ...
        if 'Var' in d:
            ...
            self.add_double_dir_edge(
                ssa_node.index, var_node.index, VAR_LINK_TYPE, VAR_LINK_TYPE + 1)
        else:
            for key in d:
    ...
    def traverse_ast(self, node, d):
        if 'Var' in d:  # var node
            ...
            self.add_double_dir_edge(
                node.index, var_node.index, AST_EDGE_TYPE, AST_EDGE_TYPE + 1)
        elif 'Const' in d:  # const node
            assert len(d) == 1  # const node
            const_node = self.const_nodes[d['Const']]
            self.add_double_dir_edge(
                node.index, const_node.index, AST_EDGE_TYPE, AST_EDGE_TYPE + 1)
        elif 'UNK' in d:  # unk node
            assert len(d) == 1  # unk node
            self.add_double_dir_edge(
                node.index, self.unk_node.index, AST_EDGE_TYPE, AST_EDGE_TYPE + 1)
        else:
            ...
            for key in d:
                if type(d[key]) is dict:
                    ...
                    self.add_double_dir_edge(
                        node.index, child.index, AST_EDGE_TYPE, AST_EDGE_TYPE + 1)
