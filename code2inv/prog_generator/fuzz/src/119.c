...
void precheck(FILE *fptr, char *buff, long long int i, long long int size,
              long long int sn) {
  int f = preflag;
...
void loopcheck(FILE *fptr, char *buff, long long int temp_i,
               long long int temp_size, long long int temp_sn, long long int i,
               long long int size, long long int sn) {
  int f = loopflag;
  aflcrash(INV(i, size, sn), loopflag);
  if (f == 0 && loopflag == 1) {
    fprintf(fptr, "LoopStart : %s : %lld, %s : %lld, %s : %lld\n", "i", temp_i,
            "size", temp_size, "sn", temp_sn);
