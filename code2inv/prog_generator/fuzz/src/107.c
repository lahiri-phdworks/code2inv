...
void precheck(FILE *file_descp, char *buff, long long int a, long long int m,
...
void loopcheck(FILE *file_descp, char *buff, long long int temp_a,
               long long int temp_m, long long int temp_j, long long int temp_k,
               long long int a, long long int m, long long int j,
               long long int k) {
  int f = loopflag;
  aflcrash(INV(a, m, j, k), loopflag);
  if (f == 0 && loopflag == 1) {
    fprintf(file_descp,
            "LoopStart : %s : %lld, %s : %lld, %s : %lld, %s : %lld\n", "a",
            temp_a, "m", temp_m, "j", temp_j, "k", temp_k);
    fprintf(file_descp,
            "LoopEnd : %s : %lld, %s : %lld, %s : %lld, %s : %lld\n", "a", a,
            "m", m, "j", j, "k", k);
    assert(0);
  }
}
...
int main() {
  ...
  for (;;) {
    ...
    snprintf(vars, 256, "%s : %lld, %s : %lld, %s : %lld, %s : %lld", "a", a,
             "m", m, "j", j, "k", k);
