...
std::string RewriterClass::simplifyNot(clang::Expr *expr, int notLevel = 0) {
  // simplify a condition from !(a && b || c) and the like such that the && and
  // || occur in the root of the condition We use de Morgan's law which states
  ...
  if (llvm::isa<UnaryOperator>(expr)) {
    ...
    if (clang::UnaryOperator::getOpcodeStr(unOp->getOpcode()) == "!") {
      auto subExpr =
          unOp->getSubExpr()->IgnoreParens()->IgnoreImplicit()->IgnoreCasts();
      ...
      clang::SourceLocation e(
          clang::Lexer::getLocForEndOfToken(_e, 0, sourceManager, lo));
    ...
    if (binOp->getOpcodeStr() == "&&") {
      ...
      res.append(simplifyNot(
          binOp->getLHS()->IgnoreCasts()->IgnoreParens()->IgnoreImplicit(),
          notLevel));
      res.append(") ");
      res.append(replacementOp);
      res.append(" (");
      res.append(simplifyNot(
          binOp->getRHS()->IgnoreCasts()->IgnoreParens()->IgnoreImplicit(),
          notLevel));
      res.append(")");
...
bool RewriterClass::VisitStmt(clang::Stmt *s) {
  if (s != nullptr &&
      context->getSourceManager().isInMainFile(s->getBeginLoc())) {
    if (isa<UnaryOperator>(s)) {
      // Either to simplify not statements or to rewrite ++ and -- operators as
      // their simplified assignment operators. eg- x++ -> x = x + 1
      ...
      CharSourceRange charSourceRange =
          Lexer::getAsCharRange(range, sourceManager, lo);
      ...
      if (isa<DeclRefExpr>(unOp->getSubExpr())) {
        ...
        clang::BeforeThanCompare<SourceLocation> isBefore(
      ...
      if (!declStmt->isSingleDecl()) {
        // expand single statement with multiple declarations into multiple
        // statements each with a single statement

        const auto &sourceManager = rewriter.getSourceMgr();
        LangOptions lo;
        clang::SourceLocation b(declStmt->getBeginLoc()),
            _e(declStmt->getEndLoc());
        clang::SourceLocation e(
            clang::Lexer::getLocForEndOfToken(_e, 0, sourceManager, lo));

        std::string orgData =
            std::string(sourceManager.getCharacterData(b),
                        sourceManager.getCharacterData(e) -
                            sourceManager.getCharacterData(b));
        // llvm::errs() << orgData << "\n";
        ...
        for (auto iter = declStmt->decl_begin(); iter != declStmt->decl_end();
          ...
          if (isa<VarDecl>(*iter)) {
            ...
            declString += varDecl->getType().getAsString() + " " +
                          varDecl->getNameAsString();

            if (varDecl->hasInit()) {
              auto init = varDecl->getInit();
              auto initRange = Lexer::getAsCharRange(init->getSourceRange(),
                                                     sourceManager, lo);
              auto initEnding = Lexer::getLocForEndOfToken(
                  initRange.getEnd(), 0, sourceManager, lo);
              auto initStr = std::string(
      ...
      auto whileCond =
          whileStmt->getCond()->IgnoreParens()->IgnoreCasts()->IgnoreImplicit();
      if (isa<IntegerLiteral>(whileCond)) {
        ...
        clang::SourceLocation b(whileCond->getBeginLoc()),
            _e(whileCond->getEndLoc());
        clang::SourceLocation e(
            clang::Lexer::getLocForEndOfToken(_e, 0, sourceManager, lo));
        ...
        rewriter.InsertText(
            beginMain, "int cond" + std::to_string(numCond) + " = " +
                           intLiteral->getValue().toString(10, true) + ";\n");
        rewriter.RemoveText(charSourceRange);
      ...
      auto forCond =
          forStmt->getCond()->IgnoreParens()->IgnoreCasts()->IgnoreImplicit();
      if (isa<IntegerLiteral>(forCond)) {
        ...
        clang::SourceLocation b(forCond->getBeginLoc()),
            _e(forCond->getEndLoc());
        clang::SourceLocation e(
            clang::Lexer::getLocForEndOfToken(_e, 0, sourceManager, lo));
      ...
      auto doCond =
          doStmt->getCond()->IgnoreParens()->IgnoreCasts()->IgnoreImplicit();
...
void RewriterClassAction::EndSourceFileAction() {
  ...
  // llvm::errs() <<
  // rewriter.getRewriteBufferFor(compilerInstance->getSourceManager().getMainFileID())
  // << "\n";
  rewriter
      .getRewriteBufferFor(compilerInstance->getSourceManager().getMainFileID())
      ->write(newBuffer);
  // llvm::errs() << "New content is\n" << newBuffer.str() << "\n\n";
}

std::unique_ptr<clang::ASTConsumer>
RewriterClassAction::CreateASTConsumer(clang::CompilerInstance &Compiler,
