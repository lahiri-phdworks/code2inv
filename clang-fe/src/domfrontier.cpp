...
void ssa_transform::DominanceFrontier::calculateFrontiers(
    clang::DomTreeNode *x, llvm::DomTreeBase<clang::CFGBlock> *dominatorTree) {
  ...
  for (auto y : x->getBlock()->succs()) {
    int idIdomY =
        dominatorTree->getNode(y)->getIDom()->getBlock()->getBlockID();
...
ssa_transform::placePhi(
    std::map<std::string, std::set<clang::CFGBlock *>> assignments,
    ssa_transform::domFrontier frontier,
    ...
    while (!W.empty()) {
      ...
      for (auto Y : frontier[dominatorTree.getNode(X)]) {
        ...
        if (hasAlready[cfgBlockY] < itercount) {
          phiPlacements[cfgBlockY].push_back(
              std::pair<std::string, std::vector<std::string>>(
                  variable,
                  std::vector<std::string>(cfgBlockY->pred_size() + 1)));
