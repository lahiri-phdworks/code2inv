...
bool SSAWriterVisitor::VisitFunctionDecl(clang::FunctionDecl *Declaration) {
  ...
  if (Declaration->isMain()) {
    ...
    cfg = CFG::buildCFG(Declaration, stmt, Context, CFG::BuildOptions())
              .release();
    ...
    */
    ...
    if(outputFileStream.is_open()) {
        ...
        llvm::outs() << "Wrote SSA graph for " << srcFile << " into " <<
    jsonFileName << "\n";
    }
     */
    if (genmode == "-ssa") {
...
/**
 * This constructs a SubSSANode depending on the expression type and the number
 * and types of children for the expr
 * @param e
  ...
  if (llvm::isa<DeclRefExpr>(e)) {
    ...
    if (llvm::isa<DeclRefExpr>(functnDeclExpr)) {
      ...
      if (subNode->parent == "unknown" ||
          subNode->parent.find("__VERIFIER_nondet_") != std::string::npos) {
        subNode->type = "UNK";
        ...
        for (unsigned int i = 0; i < callExpr->getNumArgs(); i++) {
          subNode->args.push_back(
              getSubSSANode(args[i]->IgnoreCasts()->IgnoreParens()));
        }
      }
    }
    ...
    subNode->type = "OP";
    subNode->args.push_back(
        getSubSSANode(binOp->getLHS()->IgnoreCasts()->IgnoreParens()));
    subNode->args.push_back(
        getSubSSANode(binOp->getRHS()->IgnoreCasts()->IgnoreParens()));
    ...
    subNode->type = "OP";
    subNode->args.push_back(
        getSubSSANode(unop->getSubExpr()->IgnoreCasts()->IgnoreParens()));

    if (subNode->args[0]->type == "Const" &&
        (subNode->parent == "+" || subNode->parent == "-")) {
      subNode->type = "Const";
...
/**
 * This constructs a SSANode depending on the expression type and the number and
 * types of children for the expr
 * @param e
...
std::unique_ptr<SSANode> SSAWriterVisitor::getSSANode(clang::Stmt *s) {
  ...
  if (llvm::isa<BinaryOperator>(s)) {
    ...
    if (op->getOpcodeStr() == "=") {
      ...
      ssaNode->cmd = "=";
      ssaNode->leftChild =
          getSubSSANode(op->getLHS()->IgnoreCasts()->IgnoreParens());
      ssaNode->rightChild =
          getSubSSANode(op->getRHS()->IgnoreCasts()->IgnoreParens());
    ...
    for (auto iter = declSt->decl_begin(); iter != declSt->decl_end(); iter++) {
      ...
      if (varDecl->hasInit()) {
        ...
        ssaNode->rightChild =
            getSubSSANode(varDecl->getInit()->IgnoreCasts()->IgnoreParens());
      }
    }
    ...
    if (std::string(castExpr->getCastKindName()).find("ToBoolean") !=
        std::string::npos) {
      // find parent and see if it is either an if condition or it is a
      ...
      if (!parentList.empty()) {
        ...
        if (parent != nullptr) {
          if (llvm::isa<IfStmt>(parent)) {
            ...
            ssaNode->cmdName = "if";
            ssaNode->cmd = "";
            ssaNode->rightChild = getSubSSANode(
                castExpr->IgnoreImplicit()->IgnoreCasts()->IgnoreParens());
            // llvm::errs() << "PRINTING THIS NODE:\n";
            // ssaNode->print();
            // llvm::errs() << "\n\n\n";
          } else {
 ...
 */
std::unique_ptr<SSANode>
SSAWriterVisitor::getPhiNode(std::vector<std::string> &function) {
...
/**
 * For a CFGBlock block, this function iterates over each CFGElement, gets them
 * as statements and constructs the appropriate SSANode for them and adds them
 * to the SSAGraph along with the proper control flow.
 *
 * @todo Change this function to accept a Dominator Tree node, since we can get
 * the CFGBlock from the Dominator Tree and also that the Dominator tree does
 * not have loops, which will make the visited vector unnecessary
 *
 * @param block
 * @param visited a boolean vector. visited[blockID] denotes if this block has
 * been previously visited
 */
void SSAWriterVisitor::visitSeqCFGStmt(clang::CFGBlock *block,
                                       std::vector<bool> &visited) {
    ...
    if (block->pred_empty()) {
      ...
      for (auto succ : block->succs()) {
        ssaGraph.addFlow(ssaNode->id,
                         std::to_string(succ->getBlockID()) + "_1");
      }
      ...
      for (auto phiFunc : phiFuncSet) {
        ...
        phiNode->id =
            std::to_string(block->getBlockID()) + "_" + std::to_string(stmtNo);
        if (stmtNo == blkSize) {
          ssaGraph.addFlow(phiNode->id, "EXIT");
        } else {
          ssaGraph.addFlow(phiNode->id, std::to_string(block->getBlockID()) +
                                            "_" + std::to_string(stmtNo + 1));
        }
      ...
      ssaGraph.setExitNodeAltID(std::to_string(block->getBlockID()) + "_" +
                                std::to_string(stmtNo));

      // The following loop gets all the control flows to the node ID and
      // replaces the flow destination with the EXIT id
      for (auto pair : ssaGraph.getControlFlow()) {
        if (pair.second == std::to_string(block->getBlockID()) + "_" +
                               std::to_string(stmtNo)) {
      ...
      if (!block->empty()) {
        ...
        if (!block->empty()) {
          firstStmt =
              const_cast<Stmt *>((*block)[0].getAs<CFGStmt>()->getStmt());
        }

        // Iterate over all CFGElements of block, get them as Statements and
            ...
            if (ssaNode != nullptr) {
              ...
              if (s == firstStmt) {
                ...
                if (block->pred_size() > 1) {
                  // llvm::errs() << "IS THIS BLOCK " << block->getBlockID() <<
                  // " IN ANY LOOP????\n";
                  ...
                  if (residentLoop.first == 0) {
                    ...
                    llvm::errs() << isWithin(const_cast<Stmt*>(s),
                    forStmt->getCond()) << "\n";
                     */
                    ...
                    if (isWithin(const_cast<Stmt *>(s), forStmt->getCond())) {
                      ...
                      loopNode->id = std::to_string(block->getBlockID()) + "_" +
                                     std::to_string(stmtNo);
                      ssaGraph.addFlow(loopNode->id,
                    ...
                    auto whileStmt =
                        llvm::dyn_cast<WhileStmt>(residentLoop.second);
                    /*
                    llvm::errs()
                            <<
                    ...
                    */
                    ...
                    if (s->getBeginLoc() ==
                        (*((*(doStmt->child_begin()))->child_begin()))
                            ->getBeginLoc()) {
                      auto loopNode = llvm::make_unique<SSANode>();
                      loopNode->cmdName = "Loop";
                      loopNode->id = std::to_string(block->getBlockID()) + "_" +
                ...
                for (auto phiFunc : phiFuncSet) {
                  auto phiNode = getPhiNode(phiFunc.second);
                  phiNode->id = std::to_string(block->getBlockID()) + "_" +
                                std::to_string(stmtNo);
                  ssaGraph.addFlow(phiNode->id,
                                   std::to_string(block->getBlockID()) + "_" +
              ...
              ssaNode->id = std::to_string(block->getBlockID()) + "_" +
                            std::to_string(stmtNo);
              ...
              if (stmtNo < newSizeOfBlock) {
                ...
                if (block->succ_size() == 1) {
                  if (ssaGraph.isExitNode(
                          std::to_string(
                              (*(block->succ_begin()))->getBlockID()) +
                          "_1")) {
                  ...
                  // this shows that the block has an if condition and must
                  // split into two branches a true and a false branch
                  auto trueNode = llvm::make_unique<SSANode>();
                  trueNode->cmdName = "TrueBranch";
                  trueNode->cmd = "true";
                  trueNode->id = std::to_string(block->getBlockID()) + "_" +
                                 std::to_string(stmtNo + 1);
                  ...
                  falseNode->cmd = "false";
                  falseNode->id = std::to_string(block->getBlockID()) + "_" +
                                  std::to_string(stmtNo + 2);
              ...
              // llvm::errs() << "Added node " << ssaNode->id << " and cmdName "
              // << ssaNode->cmdName << " to graph\n";
              ssaGraph.addNode(ssaNode);

              stmtNo++;
            } else {
              // ssaNode will be a nullpointer in events that do not concern the
              // SSA, like declarations llvm::errs() << "No node of block " <<
              // block->getBlockID() << " and stmt " << stmtNo << " to graph\n";
              if (stmtNo == newSizeOfBlock) {
                // if last statement
                // we now need to edit the destination nodes of all the
                // predecessors to this CFGElement we get the predecessors by
                // checking which control flows point to the current stmtNo
                ...
                // Once we get the predecessors we now redirect their flows such
                // that their control goes to the first statement of the next
                // block, or EXIT if the next case is the exit block

                for (auto succ : block->succs()) {
                  for (auto src : srcList) {
                    if (ssaGraph.isExitNode(std::to_string(succ->getBlockID()) +
                                            "_1")) {
                      ssaGraph.addFlow(src, "EXIT");
                    } else {
                      ssaGraph.addFlow(src, std::to_string(succ->getBlockID()) +
                                                "_1");
                    }
                  }
                }
        ...
        // Here the block is completely empty, signifying that this is probably
        // a block where two branches merge, and so we must take care of any phi
        // functions that may occur in this block

        // llvm::errs() << "THE BLOCK IS EMPTY\n";

        if (phiPlacementMap[block->getBlockID()].empty()) {
          // no phi functions for this block
          // We simply redirect the control flows of the block's predecessors to
