...
void precheck(FILE *fptr, char *buff, long long int sum, long long int n,
              long long int i, long long int y, long long int i2) {
  int f = preflag;
...
void loopcheck(FILE *fptr, char *buff, long long int temp_sum,
               long long int temp_n, long long int temp_i, long long int temp_y,
               long long int temp_i2, long long int sum, long long int n,
               long long int i, long long int y, long long int i2) {
  int f = loopflag;
    ...
    if (choices > 10000) {
      ...
      if ((i < n)) {
        ...
        while ((i < n) && k--) {
          ...
          {
            __asm__ __volatile__("inc %%eax;" : "=a"(i) : "a"(i));
            (i2 = pow(i, 2));
            (y = (i * (i + 1) * (2 * i + 1)) / (6));
            (sum = sum + i2);
          }
          ...
          loopcount++;
          loopcheck(fptr, vars, temp_sum, temp_n, temp_i, temp_y, temp_i2, sum,
                    n, i, y, i2);
        }
      } else {
        // post-check program
        ...
        // post-condition
        postcount++;
        postcheck(fptr, vars, ((sum == ((n * (n + 1) * (2 * n + 1)) / (6)))),
                  sum, n, i, y, i2)
      }
    }
