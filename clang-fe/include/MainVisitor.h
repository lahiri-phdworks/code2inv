...
class MainFunctionVisitor
    : public clang::RecursiveASTVisitor<MainFunctionVisitor> {
public:
  ...
  std::map<int, std::list<std::pair<std::string, std::vector<std::string>>>>
  getBlockPhiPlacementMap() {
...
private:
  ...
  /**
   * varStmtStruct- This is a structure which contains the information about a
   * variable. Each occurrence of a variable in the parsed program will have an
   * instantiation of the structure associated with it. The structure has info
   * about the original variable name, the statement it occurs in and the
  ...
  struct varStmtStruct {
    ...
    varStmtStruct(std::string varName, clang::Stmt *varStmt,
  ...
  // a map between a variable and the list of CFGBlocks where that variable is
  // assigned a new value used for placing phi functions and renaming
  ...
  std::map<clang::CFGBlock *,
           std::list<std::pair<std::string, std::vector<std::string>>>>
      phiPlacements;

  // a redundant datastructure which has to be replaced- a map between CFGBlock*
  // does not hold true after the pass is completed, so to transfer the
  // information from MainVisitor to SSAWriter, I had to map the block id to the
  ...
  /**
   * Recursively traverses all the children of stmt and gets the list of
  ...
  // used for renaming using algorithm in Fig 12 of Cytron et al.
  void search(clang::DomTreeNode *X, std::map<std::string, int> &C,
...
public:
  explicit MainFunctionConsumer(clang::ASTContext *Context)
      : Visitor(Context) {}
  ...
  MainFunctionVisitor getVisitor() { return Visitor; }
...
public:
  ...
  void setSM(clang::SourceManager &SM) { sm = &SM; }
