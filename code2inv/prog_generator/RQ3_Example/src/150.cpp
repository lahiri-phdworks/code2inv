...
void loopcheck(FILE *fptr, char *buff, long long int temp_sum,
               long long int temp_n, long long int temp_i,
               long long int temp_index, long long int sum, long long int n,
               long long int i, long long int index) {
  int f = loopflag;
...
int32_t predict(int image_index) {
  ...
  torch::jit::Module module =
      torch::jit::load("RQ3_Example/mnist_model_cpp.pt");
    ...
    if (choices > 100) {
      ...
      if (index < n) {
        ...
        while ((index < n) && k--) {
          ...
          loopcount++;
          loopcheck(fptr, vars, temp_sum, temp_n, temp_i, temp_index, sum, n, i,
                    index);
        }
      } else {
        // post-check program
