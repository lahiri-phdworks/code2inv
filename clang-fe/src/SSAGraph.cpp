...
/**
 * printAsJSON prints the SSAGraph in a JSON format into an outstream (either
 * cout or ofstream if needed to print into a file). Currently every node is
 * printed on a new line, regardless if it has more arguments.
 *
 * @todo Edit the function so that nodes without arguments can be printed on the
 * same line for conciseness.
 *
 * @param indent
 * @param outStream
 */
...
void SSAGraph::generatePrePath(std::vector<std::vector<std::string>> &path,
                               std::string nodeID, std::string terminate,
    ...
    if(command == "Loop") {
        ...
        if (predecessors.size() > 1) {
            ...
            for(auto successor : successors) {
                generatePrePath(path, visited, successor, indent,
    currentIndent);
            }
        }
    } else if(visited[nodeID] > 1) {
...
/**
 * A function to clean the SSAGraph after the entire graph is constructed.
 * Involves removing empty nodes that may have been placed, rerouting control
 * flows as needed and changing some nodes as necessary.
 *
 * @todo Remove the function and incorporate the cleaning functionalities into
 * the SSA printSeqCFGStmt.
 */
  ...
  for (const auto &node : nodes) {
    if (node.second->cmdName.empty()) {
      ...
      if (inedgeSrc.size() == 1 && outedgeDest.size() == 2) {
        // this means that src is an the condition inside the if statement
        // represented by the current node and branches out occurs in cases when
        // the if condition is just a function call, like
        ...
        for (auto &nodeToReplace : nodes) {
          if (nodeToReplace.first == *inedgeSrc.begin()) {
            ...
            if (nodeToReplace.second->leftChild != nullptr) {
              newNode->rightChild->args.push_back(
                  std::move(nodeToReplace.second->leftChild));
            }

            if (nodeToReplace.second->rightChild != nullptr) {
              newNode->rightChild->args.push_back(
                  std::move(nodeToReplace.second->rightChild));
...
std::string SSASubNode::printInLine() {
  ...
  if (type == "OP") {
    if (args.size() == 1) {
      ...
      if (parent == "==") {
        op = "=";
        return "( " + op + " " + args[0]->printInLine() + " " +
               args[1]->printInLine() + " )";
      } else if (parent == "!=") {
        op = "=";
        return "( not ( " + op + " " + args[0]->printInLine() + " " +
               args[1]->printInLine() + " ) )";
      } else if (parent == "||") {
        return "( or " + args[0]->printInLine() + " " + args[1]->printInLine() +
               " )";
      } else if (parent == "&&") {
        return "( and " + args[0]->printInLine() + " " +
               args[1]->printInLine() + " )";
      } else if (parent == "%") {
        return "( mod " + args[0]->printInLine() + " " +
               args[1]->printInLine() + " )";
      }
      return "( " + op + " " + args[0]->printInLine() + " " +
             args[1]->printInLine() + " )";
...
std::string
SSAGraph::generateSMTCond(std::vector<std::string> path, std::string indent,
...
SSAGraph::generateLastRefVars(std::vector<std::string> path) {
...
std::string
SSAGraph::getSMT(std::map<std::string, std::set<std::string>> &variables,
                 std::string smtMode) {
  std::string smt;
  ...
  if (path.size() == 1) {
    ...
    for (auto pathBranch : path) {
      ...
      //     llvm::errs() << x << "\n";
      // }
      if (std::find(pathBranch.begin(), pathBranch.end(), "EXIT") ==
          pathBranch.end()) {
        smt += indent + "( and\n";
  ...
  for (auto var : usedVars) {
    if (std::find(loopCondVars.begin(), loopCondVars.end(), var) ==
        loopCondVars.end()) {
      smt += indent + "\t( = " + var + " " + var + "! )\n";
    }
  }
  ...
  for (int i = 0; i < path.size() - 1; i++) {
    ...
    std::set_difference(usedVars.begin(), usedVars.end(), assVar.begin(),
                        assVar.end(), std::inserter(unrefVar, unrefVar.end()));
  ...
  for (auto pathBranch : assertPath) {
    ...
    if (lastRefMap.size() > 1) {
      ...
      smt += indent + "( = " + lastRefMap.begin()->first + " " +
             lastRefMap.begin()->second + ")\n";
    }
