 ...
 * SSAGraph- a class with a set of SSANodes and a set of control flows from one
 * SSANode to another.
 *
 * SSANode- a structure which contains the statement command (assign, assert,
 * if, etc.) and the arguments to that statement.
 *
 * SSASubNode- the arguments of SSANode are given by SSASubnodes, which consist
 * of a parent, type of node and a list of arguments.
 */
...
struct SSANode {
  ...
  bool operator<(const SSANode &b) const { return id < b.id; }
};
...
private:
  ...
  void generatePrePath(std::vector<std::vector<std::string>> &path,
                       std::string nodeID, std::string terminate,
                       std::set<std::string> &visited);
  std::string
  generateSMTCond(std::vector<std::string> path, std::string indent,
                  std::map<std::string, std::string> lastAssignedVar);
  std::map<std::string, std::string>
  generateLastRefVars(std::vector<std::string> path);
...
public:
   ...
   * flows as needed and changing some nodes as necessary. Should be removed
   * when the placement function is optimized enough to avoid such placement
   * errors.
   */
  void clean();

  /**
   * A function to simplify the control flow of the SSA Graph in accordance to
   * the simplification provided by Sparrow
   * @return simplified SSA Graph
   */
  SSAGraph simplify();

  void setExitNodeAltID(std::string id) { altExitId = std::move(id); }

  bool isExitNode(const std::string &id) { return altExitId == id; }
};
} // namespace ssa_transform
