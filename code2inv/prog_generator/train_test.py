...
def test_loop():
    ...
    for b in range(len(g_list)):
        g = g_list[b]
        node_embedding = node_embedding_batch[embedding_offset:
...
if __name__ == '__main__':
    ...
    if cmd_args.encoder_model == 'GNN':
        encoder = EmbedMeanField(cmd_args.embedding_size, len(
            dataset.node_type_dict), max_lv=cmd_args.s2v_level)
    elif cmd_args.encoder_model == 'LSTM':
        encoder = LSTMEmbed(cmd_args.embedding_size,
                            len(dataset.node_type_dict))
    ...
    for epoch in range(cmd_args.num_epochs):
        ...
        for k in pbar:
            ...
            g_list = dataset.sample_minibatch(
                cmd_args.rl_batchsize, replacement=True)
        ...
        if cmd_args.save_dir is not None and cmd_args.phase == 'train':
            torch.save(encoder.state_dict(), cmd_args.save_dir +
                       '/epoch-%d.encoder' % (epoch + 1))
            torch.save(decoder.state_dict(), cmd_args.save_dir +
                       '/epoch-%d.decoder' % (epoch + 1))
            torch.save(encoder.state_dict(), cmd_args.save_dir +
                       '/epoch-latest.encoder')
            torch.save(decoder.state_dict(), cmd_args.save_dir +
                       '/epoch-latest.decoder')
